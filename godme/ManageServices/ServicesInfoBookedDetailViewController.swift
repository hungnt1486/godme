...
class ServicesInfoBookedDetailViewController: BaseViewController {
    ...
    var intVote = 0
    var listBaseServiceDetail: BaseServiceInfoBookedModel?
    ...
    func setupUI(){
        if listBaseServiceDetail?.buyerInfo?.id ?? 0 == self.modelUser.userId {
            if listBaseServiceDetail?.status == "PENDING" || listBaseServiceDetail?.status == "REJECT" {
                self.section = 2
                self.status = listBaseServiceDetail?.status ?? ""
            }
        }
        self.serviceId = listBaseServiceDetail?.serviceId ?? 0
...
//        ManageServicesManager.shareManageServicesManager().getListOrderBaseServiceDetail(serviceId: serviceId) { [unowned self](response) in
//            switch response {
//
//            case .success(let data):
//                self.hideProgressHub()
//                self.listBaseServiceDetail = data
//                for item in self.listBaseServiceDetail {
...
//        }
    ...
    func rateBaseService(model: AddNewRateBaseServiceParams){
        ManageServicesManager.shareManageServicesManager().rateBaseService(model: model) {[unowned self] (response) in
            switch response {
...
extension ServicesInfoBookedDetailViewController: UITableViewDelegate, UITableViewDataSource {
...
extension ServicesInfoBookedDetailViewController: CompleteTableViewCellProtocol{
    func didComplete() {
        if intVote == 0 {
...
extension ServicesInfoBookedDetailViewController: DescriptionCarTableViewCellProtocol{
    func getDescriptionText(_ string: String) {
        self.modelBaseService.comment = string ?? ""
    }
}

extension ServicesInfoBookedDetailViewController: VoteTableViewCellProtocol{
    func didImg1() {
