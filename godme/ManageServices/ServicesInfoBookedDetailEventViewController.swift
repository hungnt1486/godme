...
class ServicesInfoBookedDetailEventViewController: BaseViewController {
    ...
    var intVote = 0
    var listEventServiceDetail: EventServiceInfoBookedModel?
    ...
    func setupUI(){
        if listEventServiceDetail?.buyerInfo?.id ?? 0 == self.modelUser.userId {
            if listEventServiceDetail?.status == "PENDING" || listEventServiceDetail?.status == "REJECT" {
                self.section = 2
                self.status = listEventServiceDetail?.status ?? ""
            }
        }
        self.serviceId = listEventServiceDetail?.serviceId ?? 0
...
//        ManageServicesManager.shareManageServicesManager().getListOrderEventServiceDetail(serviceId: serviceId) { [unowned self](response) in
//            switch response {
//
//            case .success(let data):
//                self.hideProgressHub()
...
//    }
    ...
    func rateBaseService(model: AddNewRateBaseServiceParams){
        ManageServicesManager.shareManageServicesManager().rateEventService(model: model) {[unowned self] (response) in
            switch response {
...
extension ServicesInfoBookedDetailEventViewController: UITableViewDelegate, UITableViewDataSource {
