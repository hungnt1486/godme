...
public class ViewPagerController:UIViewController {
    ...
    fileprivate func setupCurrentPageIndicator(currentIndex: Int, previousIndex: Int) {
        ...
        if options.isTabHighlightAvailable {
            ...
            self.tabsViewList[previousIndex].removeHighlight(options: self.options, index: previousIndex)
            
            UIView.animate(withDuration: 0.8, animations: {
                
                self.tabsViewList[currentIndex].addHighlight(options: self.options, index: currentIndex)
            })
        }
